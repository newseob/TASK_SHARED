...
export default function OtherRoutine() {
  const [showList, setShowList] = useState(() => {
    // localStorage에서 상태 복원
    const saved = localStorage.getItem('otherRoutine_showList');
    return saved !== null ? JSON.parse(saved) : true;
  });

  // 상태 변경 시 localStorage에 저장
  useEffect(() => {
    localStorage.setItem('otherRoutine_showList', JSON.stringify(showList));
  }, [showList]);
  ...
  return (
    <div className="rounded shadow-none bg-transparent w-full transition-opacity opacity-60">
      {/* 헤더 */}
